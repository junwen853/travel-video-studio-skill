...
- Audits shot-flow continuity so every chapter's final clip order reads as setup/movement/texture/payoff/aftertaste instead of random scenic jumps hidden by effects.
...
- Treats every `blocked*` audit status as a safe-workflow blocker, so transition/watchability gates with specific names cannot disappear from the top-level repair list.
...
- Runs a title-bridge structural audit inside the safe workflow so stale `title_cards`, missing scenic bridge media, mismatched Resolve V2 title clips, duplicate/route/date title text, and title-zone subtitle overlap block before final QA.
- Audits BGM selection and musicality so scenic/title/transition sections use auditioned, real named music with provider/license/section-fit evidence, phrase coverage, dynamics, and multi-band energy instead of hums, tones, silence, procedural pads, or placeholders.
- Converts transition decisions into Resolve-ready recipes, non-copying reference-calibrated A/B/C candidate ladders, unattended-safe default transition selections with boundary-specific reasons and proof plans, and execution-blueprint transition payloads that carry the selected candidate into Resolve markers/clip metadata; audits the whole transition motif/cadence chain, plans 2-5 shot bridge sequences, materializes those beats, proves those bridge inserts survive into the final candidate, and proves final BGM-hit/title-safe/motion-proven/pair-continuous/execution-ready transition polish metadata plus Resolve marker/readback payloads and latest candidate-chain lineage survive into the active/final Resolve blueprint.
...
- Requires the ordered transition watch reel and transition audition packets to be explicitly muted in safe workflow, final QA, repair commands, and downstream review audits, so transition review cannot leak source-camera/user audio or falsely claim BGM-only readiness.
...
- Audits scene-flow arcs so chapters become setup, movement, lived-in texture, payoff, and aftertaste/handoff sequences instead of landmark stacks or effect-hidden jumps.
- Audits final-cut smoothness so the final candidate's adjacent shots have bridge, match, breathing, stable landing, and rare motion-effect proof instead of rough hard joins.
- Audits narrative adjacency so every adjacent visual shot has a viewer-readable route, place, story-function, bridge, BGM, title, or transition reason instead of random clip stacking.
- Audits transition viewer orientation so important route, day, place, title, and ending transitions tell viewers where they are, why the film moved, and what the landing shot means after the cut.
...
- Audits transition sequence satisfaction so the ordered transition experience is muted, restrained, bridged, landed, reference-like, and free of random rotation/whip/template effects before first-draft, final QA, V14, or handoff claims.
...
- Audits first-draft satisfaction so source intake, opening/title, BGM/captions, story/rhythm, transition readiness, reference fit, route texture, and watchdown must all have zero open rows before final QA, V14, or handoff claims.
- Audits whole-film satisfaction so the final MP4 is judged as one viewer experience, aggregating opening, chapters, rhythm, BGM, captions, transitions, reference fit, director intent, route texture, watchdown, viewer friction, first-draft satisfaction, and unattended repair closure before final QA, V14, or handoff claims.
- Audits one-shot autonomy so a raw unordered folder must prove intake, source selection, story, rhythm, BGM, captions, titles, ordered muted transition watch reel/review, transition-reference readiness, rendered transition-window proof, reference-profile application, reference transition language, reference scene grammar, timeline variety, director intent, route texture, current-output editorial watchdown, repair closure, whole-film satisfaction, unattended first draft, and preflight are connected before claiming another AI can produce a V14-level first run without extra diagnosis.
- Audits reference review closure, reference profile application, and reference transition profile so multi-video learning is first based on full-film review evidence, then reaches opening, chapter, rhythm, creator-cut, transition, caption, audio, scene-grammar, and current-film bridge/breath/match/motion-balance gates instead of remaining unused analysis.
...
- `reference_review_repair_plan/reference_review_repair_plan.md`
...
- `transition_breathing_room_contract_audit.md`
...
- `transition_sequence_satisfaction_contract_audit.md`
- `whole_film_satisfaction_contract_audit.md`
- `one_shot_autonomy_contract_audit.md`
...
- `shot_flow_continuity_contract_audit.md`
- `audience_caption_contract_audit.md`
- `unattended_first_draft_contract_audit.md`
- `title_typography_plan/title_typography_plan.md`
- `title_typography_repair_plan/title_typography_repair_plan.md`
- `cover_title_contract_audit.md`
- `title_visual_proof_contract_audit.md`
- `title_bridge_contract_audit.md`
...
- transition reference-readiness contract aggregating all transition craft gates into one no-open-row handoff gate
- transition sequence satisfaction contract aggregating watch-reel, bridge, breath, landing, motion-restraint, Resolve, rendered-proof, and reference-fit issues into one viewer-sequence handoff gate
...
- first draft satisfaction contract aggregating source, opening, BGM, captions, story, rhythm, transitions, reference fit, route texture, and watchdown into a no-open-row first-draft handoff gate
- whole film satisfaction contract aggregating the current final MP4, opening, chapter flow, rhythm, BGM/captions, transitions, reference fit, director/route texture, watchdown, viewer friction, first draft satisfaction, and unattended repair closure into one viewer-experience gate
- one-shot autonomy contract aggregating raw-folder intake, source selection, story, rhythm, BGM, captions, titles, ordered muted transition watch reel/review, transition-reference readiness, rendered transition-window proof, reference-profile application, reference transition profile, reference scene grammar, timeline variety, director intent, route texture, current-output editorial watchdown, transition satisfaction, repair closure, whole-film satisfaction, unattended first-draft closure, and Resolve preflight into a no-extra-diagnosis first-run gate
...
python3 ~/.codex/skills/travel-video-studio/scripts/audit_first_draft_satisfaction_contract.py \
  --package-dir /path/to/delivery-package \
  --json
python3 ~/.codex/skills/travel-video-studio/scripts/prepare_unattended_repair_queue.py \
  --package-dir /path/to/delivery-package \
  --json
python3 ~/.codex/skills/travel-video-studio/scripts/audit_whole_film_satisfaction_contract.py \
  --package-dir /path/to/delivery-package \
  --json
python3 ~/.codex/skills/travel-video-studio/scripts/audit_one_shot_autonomy_contract.py \
  --package-dir /path/to/delivery-package \
  --json
