#!/usr/bin/env python3
"""Audit shot-to-shot transition coverage at each adjacent visual boundary."""
...
def transition_boundary(row: dict[str, Any]) -> float | None:
    ...
    for key in ("boundarySeconds", "timelineStartSeconds", "startSeconds"):
        ...
        if value is not None:
            return value
    value = as_float(candidate.get("boundarySeconds"))
    return value
...
def title_safe(row: dict[str, Any]) -> bool:
    ...
    return bool(title.get("avoidTitleOverlayCollision") is True or "suppress" in policy or "title" in policy)


def bgm_only_audio(row: dict[str, Any]) -> bool:
    candidate = row.get("transitionPolishCandidate") if isinstance(row.get("transitionPolishCandidate"), dict) else {}
    bgm = candidate.get("bgmSync") if isinstance(candidate.get("bgmSync"), dict) else {}
...
def motion_safe(row: dict[str, Any], style: str) -> bool:
    ...
    return (
        ...
        or row.get("motionEvidenceSatisfied") is True
...
def bridge_satisfied(row: dict[str, Any], style: str) -> bool:
    candidate = row.get("transitionPolishCandidate") if isinstance(row.get("transitionPolishCandidate"), dict) else {}
    return (
        style == "bridge"
        or row.get("bridgeSequenceSatisfied") is True
        or candidate.get("bridgeSequenceSatisfied") is True
        or row.get("requiresBridgeInsert") is False
    )
...
def audited_boundary(boundary: dict[str, Any], transition: dict[str, Any] | None) -> dict[str, Any]:
    ...
    audio_ok = bgm_only_audio(transition)
    ...
    if not motion_ok:
        issues.append("motion_effect_without_route_or_bridge_evidence")
    if boundary.get("category") in IMPORTANT_CATEGORIES and style == "clean_cut" and not bridge_satisfied(transition, style):
        issues.append("important_boundary_has_only_clean_cut_without_bridge")
    if hits:
    ...
    return {
        **boundary,
        ...
        "hasBgmHit": bgm_ok,
        "titleSafe": title_ok,
        "bgmOnlyAudio": audio_ok,
        "motionSafe": motion_ok,
        "bridgeSatisfied": bridge_satisfied(transition, style),
        "forbiddenHits": hits,
        "issues": issues,
    }
...
def build_report(package_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    styles = [row.get("style") for row in audited if row.get("style")]
    decorative_run = repeated_decorative_run([str(style) for style in styles])
    ...
    if blueprint_kind == "active_blueprint":
        warnings.append("audited active blueprint because no ready candidate blueprint was found")
    status = "passed" if not blockers and (not boundaries or rows) else "blocked"
    if boundaries and not rows:
        status = "blocked"
        if "no transition rows found in selected blueprint" not in blockers:
    ...
    return {
        ...
        "summary": {
            ...
            "motionSafeBoundaryCount": sum(1 for row in audited if row.get("style") in MOTION_STYLES and row.get("motionSafe") is True),
            ...
            "styleCounts": style_counts(audited),
            "categoryCounts": category_counts(audited),
        },
...
def style_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    out: dict[str, int] = {}
    for row in rows:
        style = str(row.get("style") or "missing")
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    lines = [
        "# Shot Transition Boundary Contract Audit",
        "",
    ...
    for row in (report.get("boundaryRows") or [])[:160]:
        lines.extend(
            [
                ...
                f"- Pair matched: `{row.get('pairMatched')}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit shot-to-shot transition coverage for every visual boundary.")
    ...
    write_json(package_dir / "shot_transition_boundary_contract_audit.json", report)
    write_markdown(package_dir / "shot_transition_boundary_contract_audit.md", report)
