...
def summarize_transition_reference_readiness_contract(report: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        "exists": True,
        "status": report.get("status"),
        "requiredTransitionReportCount": summary.get("requiredTransitionReportCount"),
        "passedTransitionReportCount": summary.get("passedTransitionReportCount"),
        "transitionReadinessRowCount": summary.get("transitionReadinessRowCount"),
        "p0TransitionReadinessRowCount": summary.get("p0TransitionReadinessRowCount"),
        "p1TransitionReadinessRowCount": summary.get("p1TransitionReadinessRowCount"),
        "metricIssueCount": summary.get("metricIssueCount"),
        "phaseCounts": summary.get("phaseCounts"),
        "ownerScripts": summary.get("ownerScripts"),
        "blockers": [row.get("repairId") for row in report.get("transitionReadinessRows") or []],
    }


def summarize_rendered_transition_proof_contract(report: dict[str, Any] | None) -> dict[str, Any] | None:
    if not report:
        return None
    summary = report.get("summary") if isinstance(report.get("summary"), dict) else {}
    return {
...
def summarize_transition_sequence_satisfaction_contract(report: dict[str, Any] | None) -> dict[str, Any] | None:
    if not report:
        return None
    summary = report.get("summary") if isinstance(report.get("summary"), dict) else {}
    return {
        ...
        "categoryCounts": summary.get("categoryCounts"),
        "ownerScripts": summary.get("ownerScripts"),
        "blockers": [row.get("repairId") for row in report.get("transitionSequenceRows") or []],
    }
...
def summarize_transition_watch_reel_review_contract(report: dict[str, Any] | None) -> dict[str, Any] | None:
    if not report:
        return None
    summary = report.get("summary") if isinstance(report.get("summary"), dict) else {}
    return {
        ...
        "reelRowCount": summary.get("reelRowCount"),
        "importantReelRowCount": summary.get("importantReelRowCount"),
        "passedReviewRowCount": summary.get("passedReviewRowCount"),
        "blockedReviewRowCount": summary.get("blockedReviewRowCount"),
        "uniqueFamilyCount": summary.get("uniqueFamilyCount"),
        "familyRunMax": summary.get("familyRunMax"),
        "dominantFamilyShare": summary.get("dominantFamilyShare"),
        "highIntensityRowCount": summary.get("highIntensityRowCount"),
        "highIntensityShare": summary.get("highIntensityShare"),
        "highIntensityRunMax": summary.get("highIntensityRunMax"),
        "durationDeltaSeconds": summary.get("durationDeltaSeconds"),
        "reelProbeReady": summary.get("reelProbeReady"),
        "reelHasAudio": summary.get("reelHasAudio"),
        "blockers": report.get("blockers") or [],
        "warnings": report.get("warnings") or [],
    }
    ...
    if report.get("transitionGrammarSummary"):
        ...
        lines.extend(
            [
                ...
                f"- Pre-Resolve decisions: {grammar.get('rowsWithPreResolveDecision')}",
                f"- Decision issues: {grammar.get('decisionIssueCount')}",
                f"- BGM/preview/frame evidence rows: {grammar.get('rowsWithBgmPhraseCue')} / {grammar.get('rowsWithPreviewRequirement')} / {grammar.get('rowsWithFrameSampleEvidence')}",
                f"- Motion effect evidence rows: {grammar.get('motionEffectRowsWithBridgeEvidence')} / {grammar.get('motionEffectRowCount')}",
...
def safe_workflow(args: argparse.Namespace) -> dict[str, Any]:
    ...
    transition_watch_reel_review_cmd = [
        "python3",
        str(SCRIPTS_DIR / "audit_transition_watch_reel_review_contract.py"),
        "--package-dir",
        str(package_dir),
        "--json",
    ]
    ...
    rendered_transition_proof_cmd = [
        "python3",
        str(SCRIPTS_DIR / "audit_rendered_transition_proof_contract.py"),
        "--package-dir",
        str(package_dir),
        "--json",
    ]
    steps.append(run_step("audit_rendered_transition_proof_contract", rendered_transition_proof_cmd, ok_codes={0, 2}))

    transition_reference_readiness_cmd = [
        "python3",
        str(SCRIPTS_DIR / "audit_transition_reference_readiness_contract.py"),
        "--package-dir",
        str(package_dir),
        "--json",
    ]
    steps.append(run_step("audit_transition_reference_readiness_contract", transition_reference_readiness_cmd, ok_codes={0, 2}))

    transition_sequence_satisfaction_cmd = [
        "python3",
        str(SCRIPTS_DIR / "audit_transition_sequence_satisfaction_contract.py"),
        "--package-dir",
        str(package_dir),
        "--json",
    ]
    steps.append(run_step("audit_transition_sequence_satisfaction_contract", transition_sequence_satisfaction_cmd, ok_codes={0, 2}))
...
def finish_report(args: argparse.Namespace, started: str, steps: list[dict[str, Any]], package_dir: Path | None, status: str | None = None) -> dict[str, Any]:
    ...
    rendered_transition_proof_summary = None
    transition_reference_readiness_summary = None
    transition_sequence_satisfaction_summary = None
    ...
    for step in steps:
        ...
        if step["id"] == "audit_transition_watch_reel_review_contract":
            transition_watch_reel_review_summary = summarize_transition_watch_reel_review_contract(payload)
            if transition_watch_reel_review_summary and str(transition_watch_reel_review_summary.get("status") or "").startswith("blocked"):
        ...
        if step["id"] == "audit_rendered_transition_proof_contract":
            rendered_transition_proof_summary = summarize_rendered_transition_proof_contract(payload)
            if rendered_transition_proof_summary and str(rendered_transition_proof_summary.get("status") or "").startswith("blocked"):
                blockers.extend(
                    f"Rendered transition proof blocker: {item}"
                    for item in rendered_transition_proof_summary.get("blockers") or []
                )
            if rendered_transition_proof_summary and rendered_transition_proof_summary.get("warnings"):
                warnings.extend(
                    f"Rendered transition proof warning: {item}"
                    for item in rendered_transition_proof_summary.get("warnings") or []
                )
        if step["id"] == "audit_transition_reference_readiness_contract":
            transition_reference_readiness_summary = summarize_transition_reference_readiness_contract(payload)
            if transition_reference_readiness_summary and str(transition_reference_readiness_summary.get("status") or "").startswith("blocked"):
                blockers.extend(
                    f"Transition reference readiness blocker: {item}"
                    for item in transition_reference_readiness_summary.get("blockers") or []
                )
        if step["id"] == "audit_transition_sequence_satisfaction_contract":
            transition_sequence_satisfaction_summary = summarize_transition_sequence_satisfaction_contract(payload)
            if transition_sequence_satisfaction_summary and str(transition_sequence_satisfaction_summary.get("status") or "").startswith("blocked"):
                blockers.extend(
                    f"Transition sequence satisfaction blocker: {item}"
                    for item in transition_sequence_satisfaction_summary.get("blockers") or []
                )
    ...
    if package_dir and (package_dir / "rendered_transition_proof_contract_audit.json").exists():
        rendered_transition_proof_summary = summarize_rendered_transition_proof_contract(
            load_json(package_dir / "rendered_transition_proof_contract_audit.json")
        )
    if package_dir and (package_dir / "transition_reference_readiness_contract_audit.json").exists():
        transition_reference_readiness_summary = summarize_transition_reference_readiness_contract(
            load_json(package_dir / "transition_reference_readiness_contract_audit.json")
        )
    if package_dir and (package_dir / "transition_sequence_satisfaction_contract_audit.json").exists():
        transition_sequence_satisfaction_summary = summarize_transition_sequence_satisfaction_contract(
            load_json(package_dir / "transition_sequence_satisfaction_contract_audit.json")
        )
    ...
    if package_dir and (package_dir / "transition_watch_reel_review_contract_audit.json").exists():
    ...
    report = {
        ...
        "nextManualApprovals": [
            ...
            "Review one_shot_autonomy_contract_audit.json before final QA, V14, Skill maturity, release, or handoff; blocked_one_shot_autonomy means raw intake, source selection, story, rhythm, BGM, captions, titles, transitions, rendered transition proof, repair closure, whole-film satisfaction, unattended first draft, or preflight still prevents a no-extra-diagnosis first run.",
            "Review rendered_transition_proof_contract_audit.json after final render and before transition-reference-readiness, one-shot autonomy, final QA, V14, or handoff; blocked rendered proof means the final MP4 transition windows still have black/blank frames, white flashes, raw pillarbox/vertical frames, strobe-like luma jumps, unstable landings, or no inferable final MP4.",
            ...
            "Review transition_preview_packet/transition_preview_packet.md, transition_preview_quality_contract_audit.json, transition_audition_packet/transition_audition_packet.md, transition_audition_quality_contract_audit.json, transition_watch_reel/transition_watch_reel.md, transition_watch_reel_review_contract_audit.json, transition_audition_visual_proof_contract_audit.json, transition_audition_role_integrity_contract_audit.json, and transition_storyboard_contract_audit.json before Resolve apply so important route/title/day-change transitions have generated nonblank frame evidence plus playable muted outgoing/bridge/landing MP4 proof, one ordered watch reel, sequence-level family/high-intensity restraint proof, distinct endpoint, middle-motion, and ordered segment-role proof.",
