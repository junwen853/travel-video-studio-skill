...
REQUIRED_SCRIPTS = {
    ...
    "user_regression_gates": [
        ...
        "prepare_title_typography_repair_plan.py",
...
REQUIRED_SKILL_PATTERNS = {
    ...
    "transition_motion_accent_repair_plan_rule": "prepare_transition_motion_accent_repair_plan.py",
    "transition_flow_repair_plan_rule": "prepare_transition_flow_repair_plan.py",
    ...
    "title_typography_repair_plan_rule": "prepare_title_typography_repair_plan.py",
    ...
    "first_draft_satisfaction_contract_rule": "audit_first_draft_satisfaction_contract.py",
    "first_draft_satisfaction_status_rule": "blocked_first_draft_satisfaction",
    "first_draft_satisfaction_reference_rule": "first-draft-satisfaction-contract.md",
...
REQUIRED_STYLE_PATTERNS = {
    ...
    "transition_watch_reel_review_contract": "transition-watch-reel-review-contract.md",
...
def first_draft_satisfaction_contract_evidence(package_dir: Path) -> dict[str, Any]:
    path = package_dir / "first_draft_satisfaction_contract_audit.json"
    data = load_json(path) or {}
    summary = data.get("summary") if isinstance(data, dict) and isinstance(data.get("summary"), dict) else {}
    rows = data.get("satisfactionRows") if isinstance(data, dict) and isinstance(data.get("satisfactionRows"), list) else []
    return {
        "path": str(path),
        "exists": path.exists(),
        "status": data.get("status") if isinstance(data, dict) else None,
...
def first_draft_satisfaction_contract_ready(evidence: dict[str, Any]) -> bool:
    return (
        evidence.get("exists")
        and evidence.get("status") == "passed"
        and int(evidence.get("requiredReportCount") or 0) >= 40
        and int(evidence.get("passedReportCount") or 0) == int(evidence.get("requiredReportCount") or 0)
        and int(evidence.get("satisfactionRowCount") or 0) == 0
        and int(evidence.get("p0SatisfactionRowCount") or 0) == 0
        and int(evidence.get("p1SatisfactionRowCount") or 0) == 0
        and int(evidence.get("rowCount") or 0) == 0
        and evidence.get("writesResolve") is False
        and evidence.get("queuesRender") is False
        and evidence.get("downloadsExternalAssets") is False
...
def whole_film_satisfaction_contract_evidence(package_dir: Path) -> dict[str, Any]:
    path = package_dir / "whole_film_satisfaction_contract_audit.json"
    data = load_json(path) or {}
    summary = data.get("summary") if isinstance(data, dict) and isinstance(data.get("summary"), dict) else {}
    rows = data.get("wholeFilmSatisfactionRows") if isinstance(data, dict) and isinstance(data.get("wholeFilmSatisfactionRows"), list) else []
    return {
        "path": str(path),
        "exists": path.exists(),
        "status": data.get("status") if isinstance(data, dict) else None,
...
def whole_film_satisfaction_contract_ready(evidence: dict[str, Any]) -> bool:
    return (
        evidence.get("exists")
        and evidence.get("status") == "passed"
        and int(evidence.get("requiredWholeFilmReportCount") or 0) >= 20
        and int(evidence.get("passedWholeFilmReportCount") or 0) == int(evidence.get("requiredWholeFilmReportCount") or 0)
...
def transition_reference_readiness_contract_evidence(package_dir: Path) -> dict[str, Any]:
    path = package_dir / "transition_reference_readiness_contract_audit.json"
    data = load_json(path) or {}
    summary = data.get("summary") if isinstance(data, dict) and isinstance(data.get("summary"), dict) else {}
    rows = data.get("transitionReadinessRows") if isinstance(data, dict) and isinstance(data.get("transitionReadinessRows"), list) else []
    return {
        "path": str(path),
        "exists": path.exists(),
        "status": data.get("status") if isinstance(data, dict) else None,
...
def transition_reference_readiness_contract_ready(evidence: dict[str, Any]) -> bool:
    return (
        evidence.get("exists")
        and evidence.get("status") == "passed"
        and int(evidence.get("requiredTransitionReportCount") or 0) >= 40
        and int(evidence.get("passedTransitionReportCount") or 0) == int(evidence.get("requiredTransitionReportCount") or 0)
...
def transition_watch_reel_review_contract_evidence(package_dir: Path) -> dict[str, Any]:
    path = package_dir / "transition_watch_reel_review_contract_audit.json"
    data = load_json(path) or {}
    summary = data.get("summary") if isinstance(data.get("summary"), dict) else {}
    safety = data.get("safety") if isinstance(data.get("safety"), dict) else {}
    return {
...
def transition_watch_reel_review_contract_ready(evidence: dict[str, Any]) -> bool:
    reel_rows = int(evidence.get("reelRowCount") or 0)
    return (
        evidence.get("exists")
        and evidence.get("status") in {"passed", "passed_no_important_transitions"}
        and int(evidence.get("blockedReviewRowCount") or 0) == 0
        and int(evidence.get("blockedCheckCount") or 0) == 0
        and int(evidence.get("blockerCount") or 0) == 0
        and not evidence.get("blockers")
        and (reel_rows == 0 or int(evidence.get("passedReviewRowCount") or 0) >= reel_rows)
        and (reel_rows == 0 or evidence.get("reelProbeReady") is True)
        and evidence.get("reelHasAudio") in {False, None}
        and int(evidence.get("highIntensityRunMax") or 0) <= 1
        and int(evidence.get("familyRunMax") or 0) <= 2
        and evidence.get("writesResolve") is False
        and evidence.get("queuesRender") is False
        and evidence.get("downloadsExternalAssets") is False
        and evidence.get("modifiesSourceFootage") is False
...
def build_report(package_dir: Path, skill_dir: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    first_draft_satisfaction_evidence = first_draft_satisfaction_contract_evidence(package_dir)
    add_check(
        checks,
        "First draft satisfaction contract proves the first serious draft has no open source, opening, BGM, caption, story, rhythm, transition, reference, route, or watchdown rows",
        first_draft_satisfaction_contract_ready(first_draft_satisfaction_evidence),
        first_draft_satisfaction_evidence,
    )
    whole_film_satisfaction_evidence = whole_film_satisfaction_contract_evidence(package_dir)
    add_check(
        checks,
    ...
    transition_watch_reel_review_evidence = transition_watch_reel_review_contract_evidence(package_dir)
    add_check(
        checks,
        "Transition watch reel review contract proves the ordered transition sequence is muted, timed, varied, and not high-intensity effect spam",
        transition_audition_packet_ready(transition_audition_packet)
        and transition_audition_quality_contract_ready(transition_audition_quality_evidence)
        and transition_watch_reel_ready(transition_watch_reel_evidence_report)
