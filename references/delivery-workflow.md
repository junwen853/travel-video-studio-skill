...
- `large_source_unattended_readiness_contract_audit.json` / `.md`: contract proving 100GB-class unordered folders have media-root intake, source-drive preservation, workspace free-space budget, resume checkpoints, whole-folder recognition, route truth, source selection, first assembly, unattended first-draft, and blueprint preflight connected before handoff
- `one_shot_autonomy_contract_audit.json` / `.md`: final autonomy gate proving raw-folder intake, source selection, story, rhythm, BGM, captions, titles, transitions, repair closure, whole-film satisfaction, unattended first-draft, and blueprint preflight are connected before claiming another AI can produce a V14-level first run without extra diagnosis
...
- `pacing_watchability_contract_audit.json` / `.md`: no-write audit proving the final candidate's shot-duration distribution, chapter breathing, long-hold reduction, and short-clip readability match the reference rhythm profile
- `narrative_adjacency_contract_audit.json` / `.md`: no-write audit proving every adjacent visual shot has a viewer-readable route, place, story-function, bridge, BGM, title, or transition reason instead of random clip stacking
...
- `transition_execution_blueprint/resolve_timeline_blueprint_transition_execution.json`: non-destructive Resolve candidate containing transition execution metadata, selected reference-candidate payloads, three-beat `transitionMotionExecution`, sensory-continuity plans, markers, and clip in/out transition annotations
...
- `transition_motif_coherence_contract_audit.json` / `.md`: film-level gate proving motif rows, reference-selected style families, important-boundary bridge logic, and rare motion accents agree as one coherent transition language
...
- `transition_source_coverage_contract_audit.json` / `.md`: source-level gate proving every transition row has selected outgoing, bridge, motion, and landing material before visible effects are trusted
...
- `transition_motion_direction_contract_audit.json` / `.md`: motion-direction gate proving rotation/whip/push/speed-ramp effects match source or bridge movement direction instead of random motion
- `transition_motion_accent_contract_audit.json` / `.md`: motion-accent gate proving whip, rotation, push, zoom, slide, and speed-ramp accents are rare, motivated, BGM-hit aligned, title-safe, and not back-to-back template motion
- `transition_effect_recipe_contract_audit.json` / `.md`: effect-recipe gate proving visible transitions have executable restrained Resolve keyframes, easing, parameter envelopes, BGM-only audio, BGM-hit timing, and stable landing holds
- `transition_cutpoint_contract_audit.json` / `.md`: cutpoint timing gate proving every transition has a readable outgoing tail, BGM-hit bridge/effect point, stable landing hold, source handles, title/subtitle quiet zone, and BGM-only audio
- `transition_action_anchor_contract_audit.json` / `.md`: action-anchor gate proving every transition leaves from readable source action, bridges or matches through a motivated connector, and lands on a stable shot
- `transition_sensory_continuity_contract_audit.json` / `.md`: sensory-continuity gate proving every transition carries visual continuity, BGM-only phrase-hit audio, title/subtitle quiet zones, route/mood continuity for important boundaries, motion direction when used, and stable landing proof
...
- `transition_watch_reel/transition_watch_reel.json` / `.md`: ordered muted reel combining important transition auditions so sequence rhythm, repeated effect families, and high-intensity runs are reviewable before Resolve apply
- `transition_watch_reel_review_contract_audit.json` / `.md`: automated sequence-level gate proving the ordered reel is probeable, muted, timed, varied, and not high-intensity effect spam
- `transition_watch_reel_watchdown_repair_plan/transition_watch_reel_watchdown_repair_plan.json` / `.md`: current-reel watchdown gate requiring timecoded viewer-facing decisions for flow, motivation, effect restraint, landing continuity, and BGM/caption safety before V14/final handoff
...
- `pacing_watchability_contract_audit.json` / `.md`: final-candidate pacing gate proving reference-calibrated average/median shot lengths, no overlong flat holds, no short flicker runs, and chapter-level scenic/payoff/aftertaste breathing
...
- `unattended_first_draft_contract_audit.json` / `.md`: no-write first-draft gate proving raw intake, footage select, opening/chapter story, title/cover, captions, BGM, audio policy, establishing/effects, rhythm/creator cut, chapter-story-spine, shot-flow continuity, scene-flow arc, final-cut smoothness, pacing watchability, narrative adjacency, timeline variety, transition QA, reference repair closure, unattended repair queue, and Resolve preflight are connected before handoff
...
- `bgm_musicality_contract_audit.json` / `.md`: BGM musicality gate proving the selected bed is real named music with phrase coverage, dynamics, multi-band energy, and no hum/tone/silence/placeholder failure
...
- `workflow_run_report.json` / `.md`: safe local workflow report with command outcomes, project-state summary, Resolve API summary, route decision summary, route decision application summary, asset decision summary, audience-caption, large-source unattended-readiness, unattended repair queue, transition-reference-candidates, transition-reference-selection, transition-microstructure, transition-motion-direction, transition-motion-accent, transition-effect-recipe, transition-cutpoint, transition-action-anchor, transition-sensory-continuity, transition-bridge visual evidence, transition-preview-packet, transition-audition visual proof, transition-audition role integrity, transition-storyboard, transition-breathing-room, scene-flow-arc, final-cut-smoothness, pacing-watchability, narrative-adjacency, transition-viewer-orientation, transition-scene-settlement, Resolve transition apply pending-manual summary, chapter-story-spine, shot-flow-continuity, timeline-variety, unattended-first-draft, one-shot-autonomy, Skill maturity, and V14 baseline summaries, BGM phrase blueprint and BGM musicality summaries, Resolve dry-run summary, Resolve apply contract summary, render-plan summary, audit status, safety flags, and remaining blockers
- `transition_viewer_orientation_contract_audit.json` / `.md`: viewer-orientation gate proving important route/day/place/title/ending transitions have viewer purpose, route/title/caption/BGM/bridge cue, stable landing, preview/audition evidence, and narrative handoff reason before Resolve apply
- `transition_scene_settlement_contract_audit.json` / `.md`: post-transition scene-settlement gate proving important route/day/place/title/ending transitions land into enough local texture/payoff footage before the next idea
...
- BGM musicality audit passes before Resolve apply or final QA, proving the selected bed is not a hum, sine tone, buzz, silence, placeholder, one-band audio, or flat-dynamics fake music
...
- transition choreography exists before transition execution blueprint, motif, motion-direction, cutpoint, preview, storyboard, or Resolve apply, proving every boundary has outgoing/bridge-or-motion/landing, BGM-hit, caption/title quiet-zone, restrained intensity metadata, and motion-direction plan fields that can be carried into Resolve candidate transitions
...
- transition execution blueprint report exists before Resolve apply, proving approved transition recipes became candidate `transitions[]`, clip in/out metadata, sensory-continuity plans, and timeline markers without mutating the active blueprint by default
...
- transition polish blueprint report, application audit, Resolve transition materialization audit, Resolve transition apply plan/audit, final blueprint lineage audit, transition cadence audit, transition microstructure audit, transition scene-arc audit, transition effect-palette audit, transition motif-coherence audit, transition visual-match audit, transition reference candidates, transition reference selection, transition choreography plan/contract, transition motion-direction audit, transition cutpoint audit, transition action-anchor audit, transition sensory-continuity audit, transition breathing-room audit, pacing watchability audit, narrative adjacency audit, transition viewer-orientation audit, transition scene-settlement audit, transition audition visual proof audit, transition audition role-integrity audit, and transition bridge visual evidence audit exist before Resolve apply, proving final transitions carry BGM-hit timing, title/subtitle avoidance, visual/audio/caption/route-or-mood/landing sensory continuity, motion-evidence checks, effect/landing direction match, readable outgoing tails, BGM-hit bridge/effect points, stable landing holds, source handles, readable outgoing/bridge-or-match/landing action anchors, restrained keyframes, active/final-blueprint survival after rhythm recut, Resolve marker/readback payload survival, API/manual/bridge apply-path proof, latest candidate-chain inheritance, shot-to-shot landing/handle proof, outgoing/bridge/landing scene arcs, one coherent motif language, three-beat choreography, stable post-transition landing holds, reference-calibrated final pacing, viewer-readable adjacent-shot narrative reasons, explicit route/title/caption/BGM/bridge orientation cues, enough local scene-settlement footage before the next idea, local bridge-video probe/frame evidence, audition endpoint/middle-motion frame evidence, ordered audition segment-role evidence, a whole-film palette that is neither bare cuts nor repeated effect spam, and pair-level visual reasons plus selected A/B/C defaults for every adjacent cut
...
- transition watch reel watchdown repair plan returns `ready_no_transition_watch_reel_watchdown_repairs_needed` after the current ordered muted reel is watched with concrete timecoded decisions; automated reel metrics alone are not enough for V14 or final handoff
...
- one-shot autonomy audit passes before final QA, V14, Skill maturity, release, or external handoff, proving raw intake, source selection, story, rhythm, BGM, captions, titles, transitions, repair queue, whole-film satisfaction, unattended first-draft, and Resolve preflight are all closed with no extra user diagnosis required
