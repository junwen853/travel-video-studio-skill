...
- transition polish blueprint report, application audit, Resolve transition materialization audit, Resolve transition apply plan/audit, final blueprint lineage audit, transition cadence audit, transition microstructure audit, transition scene-arc audit, transition effect-palette audit, transition visual-match audit, and transition bridge visual evidence audit exist before Resolve apply, proving final transitions carry BGM-hit timing, title/subtitle avoidance, motion-evidence checks, restrained keyframes, active/final-blueprint survival after rhythm recut, Resolve marker/readback payload survival, API/manual/bridge apply-path proof, latest candidate-chain inheritance, shot-to-shot landing/handle proof, outgoing/bridge/landing scene arcs, local bridge-video probe/frame evidence, a whole-film palette that is neither bare cuts nor repeated effect spam, and pair-level visual reasons for every adjacent cut
