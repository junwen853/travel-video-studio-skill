...
- `unattended_first_draft_contract_audit.json` / `.md`: no-write first-draft gate proving raw intake, footage select, opening/chapter story, title/cover, captions, BGM, audio policy, establishing/effects, rhythm/creator cut, chapter-story-spine, shot-flow continuity, scene-flow arc, final-cut smoothness, pacing watchability, timeline variety, transition QA, reference repair closure, unattended repair queue, and Resolve preflight are connected before handoff
