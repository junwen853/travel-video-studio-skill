...
- transition polish blueprint report and application audit exist before Resolve apply, proving final transitions carry BGM-hit timing, title/subtitle avoidance, motion-evidence checks, restrained keyframes, and active/final-blueprint survival after rhythm recut
...
- transition motivation, pair-continuity, execution-readiness, transition-polish application, bridge-sequence application, creator-cut application, and reference scene-grammar contract audits pass before Resolve apply, proving every transition has route/bridge/motion/title/BGM reasoning, every adjacent from/to shot has concrete continuity evidence, every final transition is Resolve-executable, polished transition metadata survives into the active/final blueprint, planned bridge beats survive into the final candidate, rejected/weak shots are not active in the final candidate, and the opening/chapters/ending are not a flat montage
