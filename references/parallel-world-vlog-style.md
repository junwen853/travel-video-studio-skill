...
Run `prepare_transition_watch_reel.py --package-dir <package> --build-reel` after transition audition quality; the transition watch reel must be `ready_with_transition_watch_reel` or `ready_no_important_transitions` before storyboard approval, Resolve apply, final QA, or V14 handoff.
