...
Use this engine after `prepare_transition_preview_packet.py` and `audit_transition_preview_quality_contract.py`, and before `audit_transition_storyboard_contract.py`, Resolve apply, final QA, maturity, or V14 baseline claims.
...
If the packet is `needs_audition_build`, rerun with `--build-clips`. If it is blocked, repair the upstream preview packet or bridge visual evidence first. Do not approve storyboard or Resolve apply from metadata alone.
