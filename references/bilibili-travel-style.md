...
- Before Resolve apply or final QA handoff, run `audit_large_source_unattended_readiness_contract.py`, `audit_final_blueprint_lineage_contract.py`, `audit_effect_motion_application_contract.py`, `audit_reference_profile_application_contract.py`, `audit_chapter_story_spine_contract.py`, `audit_shot_flow_continuity_contract.py`, `audit_transition_breathing_room_contract.py`, and `audit_final_source_usage_contract.py` so large unordered source folders are fully indexed/recognized/selected before style work, BGM phrase, effect motion, bridge sequence, transition execution, rhythm recut, transition polish, chapter-spine execution, shot-flow continuity, transition breathing-room, and reference-profile application work is present in the active/final blueprint and downstream plans, restrained effects are not dropped or overused, and the final raw clips still come from footage-select hero/main/texture choices rather than unmatched or utility-dominant source material.
